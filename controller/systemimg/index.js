...
module.exports = {
    ...
    updatetopathOss: async (ctx, next) => {
        const { ip, body } = ctx.request
        const extend_name = body.name
        const bitmap = body.content.split(',')[1].slice(0, -2)
        let index = bitmap.lastIndexOf('=')
        ...
        if (body.type == '5') {
            ...
            imgUrl = `/upload/${timeStamp}${extend_name.split('.')[0]}.ico`
            let obj = {
                name: '',
                type: '',
                url: '',
            ...
            await ctx.db.FileUpdateHist.create(obj)
            pngToIco(path).then(buf => {
                fse.writeFileSync(`./public/${upload}`, buf);
            }).catch(console.error);
            ctx.success({ 'url': imgUrl }, '上传成功!')
        } else {
            let path = `./public/upload/${timeStamp}${extend_name}`
            imgUrl = `/upload/${timeStamp}${extend_name}`
            let obj = {
                name: '',
                type: '',
                url: '',
        ...
        // 上传文件到储存桶
        await minio.putObject(bucketName, bucketFileName, fse.createReadStream(`./public/${imgUrl}`), () => {
            // 删除
            fse.remove(`./public/${imgUrl}`)
        })   
